...
- Setup Universal Link and App Link
   + Upload assetlinks.json to justnote.cc/.wellknown/
    ...
    ~ generate sha256 for debug.keystore, make sure these two keystores never be published
    ...
    ~ *Need to update the file for every release!*
   + Upload apple-app-site-association to justnote.cc/.wellknown
* test users
