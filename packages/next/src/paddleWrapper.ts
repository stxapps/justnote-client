...
import {
  DOMAIN_NAME, COM_JUSTNOTECC_SUPPORTER, SIGNED_TEST_STRING, PADDLE_RANDOM_ID,
  IAP_PADDLE_PRE_URL,
} from './types/const';
...
const getProduct = (productId) => {
  if (productId !== COM_JUSTNOTECC_SUPPORTER) {
    throw new Error(`In paddleWrapper.getProduct, invalid productId: ${productId}`);
  }
  return doSandbox ? 46921 : 811083;
};

const getProductId = (product) => {
  if (![46921, 811083].includes(product)) {
