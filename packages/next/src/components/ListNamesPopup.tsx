...
const ListNamesPopup = () => {
  ...
  const _render = () => {
    ...
    return (
      <div className={tailwind('flex h-full w-full flex-col')}>
        ...
        <div className={tailwind('flex h-13 w-full flex-shrink-0 flex-grow-0 flex-row items-center justify-end border-t border-gray-200 px-3 blk:border-gray-600')}>
  ...
  const panel = (
    <motion.div key="LNP_popup" style={popupStyle} className={tailwind('fixed overflow-auto rounded-md bg-white shadow-xl ring-1 ring-black/5 blk:bg-gray-800 blk:ring-white/25')} variants={popupFMV} initial="hidden" animate="visible" exit="hidden" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
      {_render()}
    </motion.div>
  );
  ...
  return (
    <AnimatePresence key="AP_lnPopup">
      <motion.button key="LNP_cancelBtn" ref={cancelBtn} onClick={onCancelBtnClick} className={tailwind('fixed inset-0 h-full w-full cursor-default bg-black/25 focus:outline-none')} variants={popupBgFMV} initial="hidden" animate="visible" exit="hidden" />
      {panel}
    </AnimatePresence>
  );
};
