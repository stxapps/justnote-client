...
const DateFormatMenuPopup = () => {
  ...
  const buttons = (
    <div className={tailwind('py-1')}>
      {NOTE_DATE_FORMATS.map((format, i) => {
        return (
          <button key={format} onClick={() => onMenuPopupBtnClick(format)} className={tailwind('block w-full truncate rounded-xs px-4 py-3 text-left text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:bg-gray-100 focus:text-gray-900 focus:outline-none blk:text-gray-200 blk:hover:bg-gray-700 blk:hover:text-gray-50 blk:focus:bg-gray-700 blk:focus:text-white')} role="menuitem">{NOTE_DATE_FORMAT_TEXTS[i]}</button>
        );
      })}
    </div>
  );

  const popupClassNames = 'fixed min-w-[6rem] overflow-auto rounded-md bg-white shadow-xl ring-1 ring-black/5 blk:bg-gray-800 blk:ring-white/25';
  ...
  if (popupSize) {
    ...
    const popupStyle: any = { ...posStyle, maxHeight };
  ...
  return (
    <AnimatePresence key="AP_dfmPopup">
      <motion.button key="DFMP_cancelBtn" ref={cancelBtn} onClick={onCancelBtnClick} className={tailwind('fixed inset-0 h-full w-full cursor-default bg-black/25 focus:outline-none')} variants={popupBgFMV} initial="hidden" animate="visible" exit="hidden" />
      {panel}
    </AnimatePresence>
  );
};
