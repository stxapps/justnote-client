...
const SidebarListNames = () => {
  ...
  return (
    <nav className={tailwind('mt-6 overflow-y-auto pl-3')}>
      {doShowSwitch && <div className={tailwind('flex items-center justify-start')}>
        <p className={tailwind('flex-shrink flex-grow truncate text-sm font-medium text-gray-500 blk:text-gray-400')}>{rootName}</p>
        <button onClick={onSwitchBtnClick} className={tailwind('group flex h-10 w-10 flex-shrink-0 flex-grow-0 items-center justify-center focus:outline-none')}>
          <svg className={tailwind('ml-2.5 h-5 w-5 rounded text-gray-500 opacity-80 group-hover:text-gray-700 group-focus-visible:ring-2 group-focus-visible:ring-gray-400 blk:text-gray-400 blk:group-hover:text-gray-200 blk:group-focus-visible:ring-gray-500')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className={tailwind('space-y-1.5 pr-1')}>
        {content}
      </div>
    </nav>
  );
...
const InnerSidebarListName = (props) => {
    ...
    if (listNameObj && listNameObj.children && listNameObj.children.length > 0) {
      ...
      expandBtn = (
        <button onClick={onExpandBtnClick} className={tailwind('group -ml-2.5 flex h-10 w-8 flex-shrink-0 flex-grow-0 items-center justify-center rounded focus:outline-none focus-visible:bg-gray-200 blk:focus-visible:bg-gray-700 lg:h-9')}>
          <div className={tailwind('ml-1 flex h-3.5 w-3.5 items-center justify-center rounded-xs text-gray-500 group-hover:text-gray-700 blk:text-gray-400 blk:group-hover:text-gray-200')}>
            {expandSvg}
          </div>
        </button>
      );
      ...
      expandBtn = (
        <div className={tailwind('-ml-2.5 h-10 w-8 flex-shrink-0 flex-grow-0 lg:h-9')} />
      );
    }
  }
  ...
  if (isChildless) {
    if (listNameObj.listName === TRASH) {
      svg = (
        <svg className={tailwind(svgClassNames)} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  return (
    <React.Fragment>
      <div style={viewStyle} className={tailwind('flex items-center justify-start')}>
        {expandBtn}
        <button onClick={onListNameBtnClick} className={tailwind(`group flex w-full min-w-0 flex-shrink flex-grow items-center rounded-md px-2 py-2.5 text-base font-medium focus:outline-none lg:py-2 lg:text-sm ${btnClassNames}`)}>
          {svg}
          <span className={tailwind('truncate')}>{listNameObj.displayName}</span>
        </button>
      </div>
...
const InnerSidebarTagName = (props) => {
  ...
  return (
    <div className={tailwind('flex items-center justify-start pl-3')}>
      <button onClick={onTagNameBtnClick} className={tailwind(`group flex w-full min-w-0 flex-shrink flex-grow items-center rounded-md px-2 py-2.5 text-base font-medium focus:outline-none lg:py-2 lg:text-sm ${btnClassNames}`)}>
        <span className={tailwind('truncate')}>{tagNameObj.displayName}</span>
      </button>
    </div>
  );
};
