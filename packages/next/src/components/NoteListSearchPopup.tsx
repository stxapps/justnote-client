  ...
  return (
    <AnimatePresence key="AP_NL_SearchPopup">
      <motion.div className={tailwind('flex h-14 items-center justify-between border-b border-gray-200 pl-4 pr-2 blk:border-gray-700 sm:pl-6 sm:pr-4 lg:hidden')} variants={popupFMV} initial="hidden" animate="visible" exit="hidden">
        <label htmlFor="noteListSearch" className={tailwind('sr-only')}>Search</label>
        <div className={tailwind('relative flex-1 rounded-md bg-white shadow-xs blk:bg-gray-900')}>
          <input ref={searchInput} onChange={onSearchInputChange} type="search" name="search" id="noteListSearch" className={tailwind('block w-full rounded-md border border-gray-300 bg-white py-1.5 pl-9 pr-6 text-base leading-5 text-gray-700 placeholder:text-gray-500 focus:border-gray-500 focus:outline-none focus:ring-gray-500 blk:border-gray-600 blk:bg-gray-900 blk:text-gray-200 blk:placeholder:text-gray-400 blk:focus:border-gray-400 blk:focus:ring-gray-400')} placeholder="Search" value={searchString} autoCapitalize="none" />
