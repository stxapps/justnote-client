...
const NavPanel = () => {
  ...
  const doRightPanelAnimateHidden = useSelector(
  ...
  return (
    <div style={{ height: safeAreaHeight }} className={tailwind(`relative w-full bg-white blk:bg-gray-900 ${preventScrollClassNames}`)}>
      ...
      <motion.div className={tailwind('absolute inset-0 flex overflow-hidden')} variants={canvasFMV} initial={false} animate={isSidebarPopupShown ? 'visible' : 'hidden'}>
        ...
        <motion.div className={tailwind('relative max-w-64 flex-1 bg-gray-100 pr-2 blk:bg-gray-800')} variants={sideBarFMV}>
          <Sidebar />
        </motion.div>
        {/* Need relative to be above absolute above */}
      ...
      {/* Right panel */}
      <motion.div className={tailwind('absolute inset-0 overflow-hidden')} variants={canvasFMV} initial={false} animate={note ? 'visible' : doRightPanelAnimateHidden ? 'hidden' : 'vanish'} onAnimationComplete={onRightPanelAnimEnd}>
