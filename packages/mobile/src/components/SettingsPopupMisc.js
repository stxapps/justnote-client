...
const SettingsPopupAccount = (props) => {
          ...
          <View style={tailwind('m-4 p-4 bg-red-50 rounded-md shadow-lg')}>
            <View style={tailwind('flex-row')}>
              ...
              <View style={tailwind('ml-3')}>
                ...
                <Text style={tailwind('mt-2.5 text-base text-red-700 font-normal')}>Please wait a moment and try again. {safeAreaWidth < SM_WIDTH ? '' : '\n'}If the problem persists, please <Text onPress={() => Linking.openURL('https://justnote.cc/support')} style={tailwind('text-base text-red-700 font-normal underline')}>contact us</Text>
                  <Svg style={tailwind('mb-2 text-base text-red-700 font-normal')} width={16} height={16} viewBox="0 0 20 20" fill="currentColor">
  ...
  const addedDTBtnInnerClassNames = sortOn === ADDED_DT ? 'text-green-800 font-normal' : 'text-gray-700 font-normal';
  const addedDTRBtnClassNames = sortOn === ADDED_DT ? 'border-green-600' : 'border-gray-200';
  const addedDTRBtnInnerClassNames = sortOn === ADDED_DT ? 'bg-green-600' : 'bg-gray-200';

  const updatedDTBtnClassNames = sortOn === UPDATED_DT ? 'bg-green-100 border-green-200' : 'border-gray-200';
  const updatedDTBtnInnerClassNames = sortOn === UPDATED_DT ? 'text-green-800 font-normal' : 'text-gray-700 font-normal';
  const updatedDTRBtnClassNames = sortOn === UPDATED_DT ? 'border-green-600' : 'border-gray-200';
  const updatedDTRBtnInnerClassNames = sortOn === UPDATED_DT ? 'bg-green-600' : 'bg-gray-200';

  const ascendingBtnClassNames = !doDescendingOrder ? 'bg-green-100 border-green-200' : 'border-gray-200';
  const ascendingBtnInnerClassNames = !doDescendingOrder ? 'text-green-800 font-normal' : 'text-gray-700 font-normal';
  const ascendingRBtnClassNames = !doDescendingOrder ? 'border-green-600' : 'border-gray-200';
  const ascendingRBtnInnerClassNames = !doDescendingOrder ? 'bg-green-600' : 'bg-gray-200';

  const descendingBtnClassNames = doDescendingOrder ? 'bg-green-100 border-green-200' : 'border-gray-200';
  const descendingBtnInnerClassNames = doDescendingOrder ? 'text-green-800 font-normal' : 'text-gray-700 font-normal';
  const descendingRBtnClassNames = doDescendingOrder ? 'border-green-600' : 'border-gray-200';
  const descendingRBtnInnerClassNames = doDescendingOrder ? 'bg-green-600' : 'bg-gray-200';
    ...
    <View style={tailwind('p-4 relative md:p-6 md:pt-4', safeAreaWidth)}>
      ...
      <View style={tailwind('mt-6 flex-row items-center justify-between md:mt-0', safeAreaWidth)}>
        <View style={tailwind('flex-grow flex-shrink')}>
          ...
          <Text style={tailwind('mt-2 text-base text-gray-700 font-normal leading-6.5')}>Allow old removed notes in Trash to be automatically deleted after 45 days</Text>
        </View>
        ...
        <View style={tailwind('sm:flex-row sm:items-start sm:justify-between', safeAreaWidth)}>
          ...
          <View style={tailwind('mt-2.5 items-center sm:ml-4 sm:flex-grow-0 sm:flex-shrink-0', safeAreaWidth)}>
            <View style={tailwind('w-full max-w-48 bg-white rounded-md shadow-sm sm:w-48', safeAreaWidth)}>
              <TouchableOpacity onPress={() => onSortOnInputChange(ADDED_DT)} style={tailwind('')}>
              ...
              <TouchableOpacity onPress={() => onSortOnInputChange(UPDATED_DT)} style={tailwind('')}>
