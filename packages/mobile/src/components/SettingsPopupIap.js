    ...
    if (purchaseStatus === null) {
      ...
      actionPanel = (
        <View style={tailwind('mt-6 justify-start items-start')}>
          <TouchableOpacity onPress={onRequestBtnClick} style={tailwind('px-3.5 py-1.5 border border-gray-400 rounded-md bg-white shadow-sm')}>
...
const IapPurchased = (props) => {
  ...
  if (purchase.source === APPSTORE) {
    ...
    appStoreLink = (
      <Text onPress={() => Linking.openURL('https://play.google.com/store/account/subscriptions?sku=com.justnotecc.supporter&package=com.justnotecc')} style={tailwind('text-base text-gray-500 font-normal leading-6.5 underline')}>Google Play</Text>
    );
  }
  ...
  if (restoreStatus === null) {
    actionPanel = (
      <View style={tailwind('mt-7 mb-4 justify-start items-start')}>
        <TouchableOpacity onPress={onRestoreBtnClick} style={tailwind('px-3.5 py-1.5 border border-gray-400 rounded-md bg-white shadow-sm')}>
