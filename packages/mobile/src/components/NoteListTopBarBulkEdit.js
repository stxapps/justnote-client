    ...
    <View style={tailwind('h-16 w-full flex-shrink-0 flex-grow-0 border-b border-gray-200')}>
      <View style={tailwind('h-full w-full flex-row justify-between sm:pl-3 sm:pr-6')}>
        <TouchableOpacity onPress={onBulkEditCancelBtnClick} style={tailwind('h-full justify-center px-4')}>
          <Svg width={24} height={24} style={tailwind('font-normal text-gray-500')} viewBox="0 0 24 24" fill="currentColor">
        ...
        <View style={tailwind('h-full flex-1 flex-row items-center justify-between sm:pl-2')}>
          <View style={tailwind('flex-shrink flex-grow')}>
            <Text style={tailwind('text-lg font-medium leading-6 text-gray-900')} numberOfLines={1} ellipsizeMode="tail">{selectedNoteIdsLength} Selected</Text>
          </View>
