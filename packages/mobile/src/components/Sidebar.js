...
const Sidebar = () => {
  ...
  const derivedUserImage = useMemo(() => {
    if (userImage) {
      return (
        <FastImage style={tailwind('h-10 w-10 flex-shrink-0 overflow-hidden rounded-full bg-gray-300')} source={{ uri: userImage }} />
      );
    }

    return (
      <Svg width={40} height={40} style={tailwind('flex-shrink-0')} viewBox="0 0 96 96">
        <Circle cx="48" cy="48" r="48" fill="rgb(229, 231, 235)" />
  ...
  return (
    <View style={tailwind('h-full w-full min-w-56 bg-gray-100 pt-5 pb-4')}>
      ...
      <View style={tailwind('flex-1')}>
        ...
        <View style={tailwind('mt-6 hidden pl-3 pr-1')}>
          <TouchableOpacity ref={profileBtn} onPress={onProfileBtnClick} style={tailwind('w-full rounded-md bg-gray-100 px-3.5 py-2 text-sm font-medium text-gray-700')}>
            <View style={tailwind('w-full flex-row items-center justify-between')}>
              <View style={tailwind('flex-1 flex-row items-center justify-between')}>
                ...
                <View style={tailwind('ml-3 flex-1')}>
                  <Text style={tailwind('text-center text-sm font-medium text-gray-900')} numberOfLines={1} ellipsizeMode="tail">{derivedUsername}</Text>
                </View>
              </View>
              <Svg width={20} height={20} style={tailwind('flex-shrink-0 font-normal text-gray-400')} viewBox="0 0 20 20" fill="currentColor">
