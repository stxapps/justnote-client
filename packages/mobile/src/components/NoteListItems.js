...
const NoteListItems = () => {
  ...
  const renderEmpty = useCallback(() => {
    ...
    if (searchString !== '') {
      return (
        <View style={tailwind('px-4 py-4 sm:px-6 sm:py-6')}>
          <Text style={tailwind('text-base font-normal text-gray-600')}>Your search - <Text style={tailwind('text-lg font-medium text-gray-800')}>{searchString}</Text> - did not match any notes.</Text>
          <Text style={tailwind('pt-4 text-base font-normal text-gray-500 sm:pt-6')}>Suggestion:</Text>
          <View style={tailwind('pt-2 pl-2')}>
            ...
            <Text style={tailwind('pt-1 text-base font-normal text-gray-500')}>{'\u2022'}  Try different keywords.</Text>
            <Text style={tailwind('pt-1 text-base font-normal text-gray-500')}>{'\u2022'}  Try more general keywords.</Text>
          </View>
        </View>
      );
    }
    ...
    if (listName === MY_NOTES) {
      return (
        <View style={tailwind('mt-32 mb-24 w-full items-center px-4 sm:px-6')}>
          <View style={tailwind('h-20 w-20 items-center justify-center rounded-full bg-gray-200')}>
            <Svg width={40} height={40} style={tailwind('font-normal text-gray-500')} viewBox="0 0 40 40" fill="currentColor">
          ...
          <Text style={tailwind('mt-6 text-center text-base font-semibold tracking-wide text-gray-800 lg:text-sm')}>No notes in {displayName}</Text>
          <Text style={[tailwind('mt-4 text-center text-sm font-normal tracking-wide text-gray-500'), { lineHeight: 22 }]}>Tap <Text style={tailwind('text-sm font-semibold text-gray-600')}>"+ New Note"</Text> button to add a new note</Text>
        </View>
      );
    }
    ...
    if (listName === TRASH) {
      return (
        <View style={tailwind('mt-32 mb-24 w-full items-center px-4 sm:px-6')}>
          <View style={tailwind('h-20 w-20 items-center justify-center rounded-full bg-gray-200')}>
            <Svg width={40} height={40} style={tailwind('font-normal text-gray-500')} viewBox="0 0 20 20" fill="currentColor">
          ...
          <Text style={tailwind('mt-6 text-center text-base font-semibold tracking-wide text-gray-800 lg:text-sm')}>No notes in {displayName}</Text>
          <Text style={[tailwind('mt-4 text-center text-sm font-normal tracking-wide text-gray-500'), { lineHeight: 22 }]}>Tap <Text style={tailwind('text-sm font-semibold text-gray-600')}>"Remove"</Text> from the menu to move notes you don't need anymore here</Text>
        </View>
      );
    }
    ...
    return (
      <View style={tailwind('mt-32 mb-24 w-full items-center px-4 sm:px-6')}>
        ...
        <Text style={tailwind('mt-6 text-center text-base font-semibold tracking-wide text-gray-800 lg:text-sm')}>No notes in {displayName}</Text>
        <Text style={[tailwind('mt-4 text-center text-sm font-normal tracking-wide text-gray-500'), { lineHeight: 22 }]}>Tap <Text style={tailwind('text-sm font-semibold text-gray-600')}>{textName}</Text> from the menu to move notes here</Text>
      </View>
    );
  ...
  const renderFetchMoreBtn = useCallback(() => {
    return (
      <View style={tailwind('my-6 px-4 sm:px-6')}>
        <TouchableOpacity onPress={onFetchMoreBtnClick} style={tailwind('w-full items-center rounded-md border border-gray-300 bg-white px-4 py-2 shadow-sm')}>
  ...
  const renderUpdateFetchedBtn = useCallback(() => {
    return (
      <View style={tailwind('my-6 px-4 sm:px-6')}>
        <TouchableOpacity onPress={onUpdateFetchedBtnClick} style={tailwind('w-full items-center rounded-md border border-gray-300 bg-white px-4 py-2 shadow-sm')}>
