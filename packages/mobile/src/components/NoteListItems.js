...
const FETCHING_MORE = 'FETCHING_MORE';
...
const NoteListItems = () => {
  ...
  const data = useMemo(() => {
    ...
    let data = [];
    if (x === 2) data = [
      { id: '1', title: 'A very very long long long long First note', text: 'Tenetur libero voluptatem rerum occaecati qui est molestiae exercitationem. Voluptate quisquam iure assumenda consequatur ex et recusandae. Alias consectetur voluptatibus. Accusamus a ab dicta et. Consequatur quis dignissimos voluptatem nisi.', createdDt: '1882181' },
      { id: '2', title: 'My second note', text: 'This is a test. That is a book.', createdDt: '192882' },
      { id: '3', title: 'Holy Shit! I did it.', text: 'WTF. Why you whining like a girl?', createdDt: '2082181' },
    ];

    if (hasMore) data.push({ id: HAS_MORE });
  ...
  const renderEmpty = useCallback(() => {
    ...
    if (searchString !== '') {
      return (
        <View style={tailwind('pt-4 px-4 sm:pt-6 sm:px-6 lg:px-8', safeAreaWidth)}>
    ...
    if (listName === MY_NOTES) {
      return (
        <View style={tailwind('w-full h-full')}>
          <View style={[tailwind('w-full items-center px-4 sm:px-6 lg:px-8', safeAreaWidth), { paddingTop: 128 }]}>
            ...
            <Text style={tailwind('mt-6 text-sm font-semibold text-gray-800 text-center tracking-wide')}>No notes in {displayName}</Text>
            <Text style={[tailwind('mt-4 text-sm text-gray-600 font-normal text-center tracking-wide'), { lineHeight: 22 }]}>Tap <Text style={tailwind('text-sm text-gray-600 font-semibold')}>"+ New Note"</Text> button to add a new note</Text>
          </View>
        </View>
      );
    }
      ...
      return (
        <View style={tailwind('w-full h-full')}>
          <View style={[tailwind('w-full items-center px-4 sm:px-6 lg:px-8', safeAreaWidth), { paddingTop: 128 }]}>
            ...
            <Text style={tailwind('mt-6 text-sm font-semibold text-gray-800 text-center tracking-wide')}>No notes in {displayName}</Text>
            <Text style={[tailwind('mt-4 text-sm text-gray-600 font-normal text-center tracking-wide'), { lineHeight: 22 }]}>Tap <Text style={tailwind('text-sm text-gray-600 font-semibold')}>"Remove"</Text> from the menu to move notes you don't need anymore here</Text>
          </View>
        </View>
      )
    }
    ...
    return (
      <View style={tailwind('w-full h-full')}>
        <View style={[tailwind('w-full items-center px-4 sm:px-6 lg:px-8', safeAreaWidth), { paddingTop: 128 }]}>
          ...
          <Text style={tailwind('mt-6 text-sm font-semibold text-gray-800 text-center tracking-wide')}>No notes in {displayName}</Text>
          <Text style={[tailwind('mt-4 text-sm text-gray-600 font-normal text-center tracking-wide'), { lineHeight: 22 }]}>Tap <Text style={tailwind('text-sm text-gray-600 font-semibold')}>{textName}</Text> from the menu to move notes here</Text>
        </View>
      </View>
    );
  }, [listName, listNameMap, searchString]);

  const renderFetchMoreBtn = () => {
    return (
      <View style={tailwind('my-6 px-4 sm:px-6 lg:px-8', safeAreaWidth)}>
        <TouchableOpacity onPress={onFetchMoreBtnClick} style={tailwind('w-full px-4 py-2 border border-gray-300 shadow-sm rounded-md bg-white items-center')}>
          <Text style={tailwind('text-sm font-medium text-gray-600')}>More</Text>
        </TouchableOpacity>
      </View>
    );
  };
  ...
  const renderItem = useCallback(({ item }) => {
    if (item.id === HAS_MORE) return renderFetchMoreBtn();
    if (item.id === FETCHING_MORE) return renderFetchingMore();
