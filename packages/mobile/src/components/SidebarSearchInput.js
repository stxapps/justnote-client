...
const SidebarSearchInput = () => {
  ...
  const searchInputStyle = Platform.OS === 'ios' ? { lineHeight: 18 } : {};
  ...
  return (
    <View style={tailwind('hidden pl-3 pr-1 mt-5 lg:flex', safeAreaWidth)}>
      <View style={tailwind('mt-1 rounded-md bg-white shadow-sm')}>
        ...
        <TextInput onChange={onSearchInputChange} style={[tailwind('w-full pl-9 pr-6 text-sm font-normal text-gray-700 border border-gray-300 rounded-md'), searchInputStyle]} placeholder="Search" placeholderTextColor="rgba(113, 128, 150, 1)" value={searchString} autoCapitalize="none" autoCompleteType="off" autoCorrect={false} />
