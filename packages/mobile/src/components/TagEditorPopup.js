...
import {
  updateTagEditor, addTagEditorTagName, updateTagData,
} from '../actions/chunk';
import {
...
const TagEditorPopup = () => {
  ...
  if (tagEditor.mode === NOT_SUPPORTED) {
    return (
      <View style={[tailwind('absolute inset-0 flex-row items-start justify-center'), canvasStyle]}>
        <TouchableWithoutFeedback onPress={onPopupCloseBtnClick}>
          <Animated.View style={[tailwind('absolute inset-0 bg-black bg-opacity-25'), bgStyle]} />
        </TouchableWithoutFeedback>
        <Animated.View style={[tailwind('mt-14 w-full overflow-hidden rounded-lg bg-yellow-50 p-4 shadow-xl md:mt-2.5'), popupStyle]}>
          <View style={tailwind('flex-row')}>
            <View style={tailwind('flex-shrink-0 flex-grow-0')}>
              <Svg style={tailwind('font-normal text-yellow-400')} width={24} height={24} viewBox="0 0 20 20" fill="currentColor">
          ...
          <TouchableOpacity onPress={onPopupCloseBtnClick} style={tailwind('absolute top-1 right-1 rounded-md bg-yellow-50 p-1')}>
            <Svg style={tailwind('font-normal text-yellow-500')} width={20} height={20} viewBox="0 0 20 20" fill="currentColor">
        ...
        <ScrollView style={{ maxHeight: panelHeight }} keyboardShouldPersistTaps="handled">
          <View style={tailwind('px-4 pt-8 pb-4 sm:px-6 sm:pb-6')}>
            ...
            <View style={tailwind(`flex-row items-center justify-start ${tagEditor.msg ? '' : 'pt-5'}`)}>
              <TextInput onChange={onDnInputChange} onSubmitEditing={onDnInputKeyPress} style={[tailwind('flex-1 rounded-md border border-gray-400 bg-white px-3.5 text-sm font-normal text-gray-700 blk:border-gray-500 blk:bg-gray-800 blk:text-gray-200'), inputStyle]} placeholder="Add a new tag" placeholderTextColor={themeMode === BLK_MODE ? 'rgb(156, 163, 175)' : 'rgb(107, 114, 128)'} value={tagEditor.displayName} />
              <TouchableOpacity onPress={onAddBtnClick} style={[tailwind('ml-2 flex-shrink-0 flex-grow-0 flex-row items-center rounded-md border border-gray-400 bg-white pl-1.5 pr-2.5 blk:border-gray-500 blk:bg-gray-800'), { paddingVertical: 5 }]}>
            ...
            <View style={tailwind('mt-6 flex-row items-center justify-start border-t border-gray-200 pt-3.5 blk:border-gray-600 sm:pt-5')}>
              <TouchableOpacity onPress={onSaveBtnClick} style={[tailwind('items-center justify-center rounded-md bg-gray-800 px-4 blk:bg-gray-100'), { paddingTop: 7, paddingBottom: 7 }]}>
