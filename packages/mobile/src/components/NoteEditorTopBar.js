...
const NoteEditorTopBar = (props) => {
  ...
  const renderFocusedCommands = () => {
    return (
      <React.Fragment>
        <TouchableOpacity onPress={onCancelBtnClick} style={tailwind('hidden rounded-md border border-white bg-white px-2 py-2 lg:flex')}>
          <Text style={tailwind('text-sm font-normal text-gray-500')}>Cancel</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={onSaveBtnClick} style={tailwind('h-full flex-row items-center border border-white bg-white pl-1 pr-1 lg:ml-3 lg:rounded-md lg:border-gray-300 lg:px-2 lg:py-2 lg:shadow-sm')}>
          <View style={tailwind('rounded p-2 lg:p-0')}>
            <Svg width={20} height={20} style={tailwind('font-normal text-green-600')} viewBox="0 0 20 20" fill="currentColor">
          ...
          <Text style={tailwind('hidden text-sm font-normal text-gray-500 lg:ml-1 lg:flex')}>Save</Text>
        </TouchableOpacity>
      </React.Fragment>
    );
  };
  ...
  return (
    <View style={tailwind('h-16 w-full flex-shrink-0 flex-grow-0 border-b border-gray-200')}>
      <ScrollView horizontal={true}>
        <View style={[tailwind('h-full flex-row justify-between lg:items-center lg:px-3'), style]}>
          <View style={tailwind('flex-row')}>
            <TouchableOpacity onPress={onRightPanelCloseBtnClick} style={tailwind('h-full justify-center rounded-md bg-white px-4 lg:hidden')}>
              {note.id !== NEW_NOTE && isEditorFocused ?
                <Svg width={20} height={20} style={tailwind('font-normal text-gray-500')} stroke="currentColor" fill="none" viewBox="0 0 24 24">
            ...
            <TouchableOpacity onPress={onToggleFullScreen} style={tailwind('hidden items-center rounded-md border border-gray-300 bg-white px-2 py-2 shadow-sm lg:flex lg:flex-row')}>
              ...
              <Text style={tailwind('text-sm font-normal text-gray-500')}>{isFullScreen ? 'Exit' : 'Expand'}</Text>
            </TouchableOpacity>
          </View>
          <View style={tailwind('flex-row')}>
            {commands}
