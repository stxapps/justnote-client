...
import { HASH_SUPPORT, DIED_UPDATING, SM_WIDTH } from '../types/const';
...
const SettingsErrorPopup = () => {
  ...
  return (
    <View style={tailwind('absolute top-14 inset-x-0 flex-row justify-center items-start bg-transparent shadow-xl md:top-0', safeAreaWidth)}>
        ...
        <View style={tailwind('m-4 p-4 bg-red-50 rounded-md shadow-lg')}>
          <View style={tailwind('flex-row')}>
            ...
            <View style={tailwind('ml-3 flex-grow flex-shrink lg:mt-0.5', safeAreaWidth)}>
              ...
              <Text style={tailwind('mt-2.5 text-sm text-red-700 font-normal leading-6')}>Please wait a moment and try again. {safeAreaWidth < SM_WIDTH ? '' : '\n'}If the problem persists, please <Text onPress={() => Linking.openURL('https://justnote.cc/' + HASH_SUPPORT)} style={tailwind('text-sm text-red-700 font-normal leading-6 underline')}>contact us</Text>
