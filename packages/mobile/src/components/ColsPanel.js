...
const ColsPanel = () => {
  ...
  return (
    <View {...viewPanResponder.panHandlers} style={tailwind('flex-1 flex-row overflow-hidden bg-white')}>
      <View style={[{ width: state.pane1Width }, tailwind(`overflow-hidden bg-white ${pane1Classes}`)]}>
        <Sidebar />
      </View>
      <View {...leftPanResponder.panHandlers} style={tailwind(`w-2 overflow-visible bg-gray-100 ${resizer1Classes}`)}>
        <TouchableOpacity onPress={onTogglePane1Shown} style={tailwind('absolute right-0 bottom-8 h-7 w-5 justify-center rounded-tl rounded-bl bg-gray-300 shadow-sm')}>
      ...
      <View style={[{ width: state.pane2Width }, tailwind(`overflow-hidden bg-white ${pane2Classes}`)]}>
        <NoteList />
        {!state.isPane1Shown && <TouchableOpacity onPress={onTogglePane1Shown} style={tailwind('absolute left-0 bottom-8 h-7 w-5 justify-center rounded-tr rounded-br bg-gray-300 shadow-sm')}>
          <Svg width={20} height={20} style={tailwind('font-normal text-gray-400')} viewBox="0 0 20 20" fill="currentColor">
      ...
      <View {...rightPanResponder.panHandlers} style={tailwind(`overflow-visible border-l border-gray-100 bg-white pr-1 ${resizer2Classes}`)} />
      <View style={tailwind('flex-1 overflow-hidden bg-white')}>
