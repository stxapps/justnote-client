...
const ColsPanel = () => {
  ...
  return (
    <View {...viewPanResponder.panHandlers} style={tailwind('flex-1 flex-row overflow-hidden bg-white')}>
      ...
      <View style={[{ width: state.pane2Width }, tailwind(`bg-white overflow-hidden ${pane2Classes}`)]}>
        <NoteList />
        {!state.isPane1Shown && <TouchableOpacity onPress={onTogglePane1Shown} style={tailwind('absolute left-0 bottom-8 w-5 h-7 bg-gray-300 rounded-tr rounded-br justify-center shadow-sm')}>
          <Svg width={20} height={20} style={tailwind('text-gray-400 font-normal')} viewBox="0 0 20 20" fill="currentColor">
