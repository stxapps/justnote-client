...
import {
  DOMAIN_NAME, HASH_SUPPORT, MERGING, DIED_MERGING, LG_WIDTH,
} from '../types/const';
        ...
        <Animated.View style={[tailwind('m-4 rounded-md bg-red-50 p-4 shadow-lg'), mergeErrorStyle]}>
          <View style={tailwind('flex-row')}>
            ...
            <View style={tailwind('ml-3 lg:mt-0.5')}>
              ...
              <Text style={tailwind('mt-2.5 text-sm font-normal text-red-700')}>Please wait a moment and try again.{'\n'}If the problem persists, please <Text onPress={() => Linking.openURL(DOMAIN_NAME + '/' + HASH_SUPPORT)} style={tailwind('text-sm font-normal text-red-700 underline')}>contact us</Text>
                <Svg width={16} height={16} style={tailwind('mb-2 font-normal text-red-700')} viewBox="0 0 20 20" fill="currentColor">
    ...
    <View style={tailwind('h-full w-full bg-white')}>
      <ScrollView>
        <View style={[tailwind('px-4 pb-4 sm:px-6 sm:pb-6'), style]}>
          ...
          <Text style={tailwind('pt-5 text-lg font-medium text-gray-800')}>{conflictedNote.notes.length} Versions found</Text>
          <Text style={tailwind('text-sm font-normal text-gray-500')}>Please choose the correct version of this note.</Text>
          ...
          <View style={tailwind('absolute top-0 left-0 lg:hidden')}>
            <TouchableOpacity onPress={onRightPanelCloseBtnClick} style={tailwind('bg-white px-4 py-4')}>
              <Svg width={20} height={20} style={tailwind('font-normal text-gray-500')} viewBox="0 0 20 20" fill="currentColor">
...
const _ConflictItem = (props) => {
  ...
  return (
    <View style={tailwind('mt-6 rounded-lg border border-gray-200')}>
      <View style={tailwind(`rounded-t-lg bg-gray-50 sm:flex-row sm:items-start sm:justify-between ${!isOpen ? 'rounded-b-lg' : ''}`)}>
        <View style={tailwind('sm:flex-shrink sm:flex-grow')}>
          <TouchableOpacity onPress={onOpenBtnClick} style={tailwind('w-full flex-row rounded-lg pt-3 pl-2')}>
            ...
            <View style={tailwind('ml-1')}>
              <Text style={tailwind('text-left text-base font-medium text-gray-800 lg:text-sm')}>Last update on {updatedDTStr}</Text>
              <Text style={tailwind('mt-1 text-left text-sm font-normal text-gray-600')}>In {getListNameDisplayName(listName, listNameMap)}</Text>
            </View>
          </TouchableOpacity>
        </View>
        <View style={tailwind('py-3 pl-2.5 sm:flex-shrink-0 sm:flex-grow-0 sm:pl-6 sm:pr-4')}>
          <TouchableOpacity onPress={onChooseBtnClick} style={tailwind('flex-row items-center self-start rounded-md border border-gray-300 bg-white px-4 py-2 shadow-sm')}>
            <Svg width={20} height={20} style={tailwind('mr-1 font-normal text-gray-500')} viewBox="0 0 20 20" fill="currentColor">
            ...
            <Text style={tailwind('text-sm font-normal text-gray-500')}>Choose</Text>
          </TouchableOpacity>
        </View>
      </View>
