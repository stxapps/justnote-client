...
const Landing = React.memo(() => {
  ...
  return (
    <ScrollView contentContainerStyle={tailwind('min-h-full')}>
      ...
      <View style={cache('L_middleView', [tailwind('max-w-lg self-center flex-1 items-start justify-center p-4 md:p-6 lg:p-8', windowWidth), { minHeight: 256 }])}>
      ...
      <View style={tailwind('items-center justify-center p-4 md:p-6 lg:p-8', windowWidth)}>
        <TouchableOpacity onPress={onSignUpBtnClick} style={tailwind(`flex-row items-center justify-center px-8 py-3 border border-transparent rounded-md shadow bg-green-600 ${windowWidth < SM_WIDTH ? 'w-full' : ''}`)}>
