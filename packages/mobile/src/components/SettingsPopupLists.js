    ...
    if (listNameObj && [MY_NOTES, TRASH, ARCHIVE].includes(listNameObj.listName)) {
      ...
      deleteBtn = (
        <TouchableOpacity onPress={onDeleteBtnClick} style={tailwind('flex-grow-0 flex-shrink-0 flex-row justify-start items-center w-8 h-10')}>
          <Svg style={tailwind('text-base text-gray-500 font-normal rounded-sm')} width={14} height={16} viewBox="0 0 14 16" fill="currentColor">
  ...
  return (
    <View style={tailwind('mt-1 flex-row justify-start items-center')}>
      {(state.mode === MODE_VIEW && listNameObj === null) && <TouchableOpacity onPress={onAddBtnClick} style={tailwind('flex-grow-0 flex-shrink-0 flex-row justify-start items-center w-8 h-10')}>
        <Svg style={tailwind('text-base text-gray-500 font-normal rounded-sm')} width={14} height={14} viewBox="0 0 14 14" fill="currentColor">
      ...
      {state.mode === MODE_EDIT && <TouchableOpacity onPressIn={onCancelBtnPress} onPress={onCancelBtnClick} style={tailwind('flex-grow-0 flex-shrink-0 flex-row justify-start items-center w-8 h-10')}>
        <Svg style={tailwind('text-base text-gray-500 font-normal rounded-sm')} width={12} height={12} viewBox="0 0 12 12" fill="currentColor">
      ...
      <View style={tailwind('flex-grow flex-shrink')}>
        <TextInput ref={input} onFocus={onInputFocus} onBlur={onInputBlur} onChange={onInputChange} onSubmitEditing={onInputKeyPress} style={tailwind('px-0 py-1 w-full bg-white text-base text-gray-600 font-normal border-0')} placeholder="Create new list" value={state.value} autoCapitalize="none" autoCompleteType="off" autoCorrect={false} editable={!(isBusy || doRetry)} />
      ...
      {state.mode === MODE_EDIT && <TouchableOpacity onPressIn={onOkBtnPress} onPress={onOkBtnClick} style={tailwind('flex-grow-0 flex-shrink-0 justify-center items-center w-10 h-10')}>
        <Svg style={tailwind('text-base text-gray-500 font-normal rounded-sm')} width={16} height={12} viewBox="0 0 14 10" fill="currentColor">
      ...
      {(state.mode === MODE_VIEW && listNameObj !== null && !doRetry) && <TouchableOpacity onPress={onEditBtnClick} style={tailwind('flex-grow-0 flex-shrink-0 justify-center items-center w-10 h-10')} disabled={isBusy}>
        <Svg style={tailwind('text-base text-gray-500 font-normal rounded-sm')} width={16} height={16} viewBox="0 0 14 14" fill="currentColor">
      ...
      {(state.mode === MODE_VIEW && listNameObj !== null && doRetry) && <TouchableOpacity onPress={onRetryRetryBtnClick} style={tailwind('flex-grow-0 flex-shrink-0 justify-center items-center w-20 h-10')} disabled={isBusy}>
        <View style={[tailwind('justify-center items-center bg-white border border-gray-300 rounded-md shadow-sm'), { height: 28, paddingLeft: 10, paddingRight: 10 }]}>
          <Text style={tailwind('text-base text-gray-500 font-normal')}>Retry</Text>
        </View>
      </TouchableOpacity>}
      {(state.mode === MODE_VIEW && listNameObj !== null && doRetry) && <TouchableOpacity onPress={onRetryCancelBtnClick} style={tailwind('flex-grow-0 flex-shrink-0 justify-center items-center w-16 h-10')} disabled={isBusy}>
        <Text style={tailwind('text-base text-gray-500 font-normal rounded-sm')}>Cancel</Text>
      </TouchableOpacity>}
    </View>
  );
};
