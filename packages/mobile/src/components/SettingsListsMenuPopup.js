...
const SettingsListsMenuPopup = () => {
  ...
  const buttons = (
    <View style={tailwind('py-1')}>
      ...
      {(isUserSignedIn && lockStatus === null) && <TouchableOpacity onPress={onAddLockBtnClick} style={tailwind('w-full flex-row items-center py-3 pl-3.5 pr-4')}>
      ...
      {(isUserSignedIn && lockStatus !== null) && <TouchableOpacity onPress={onRemoveLockBtnClick} style={tailwind('w-full flex-row items-center py-3 pl-3.5 pr-4')}>
        <Svg style={tailwind('mr-3 font-normal text-gray-400 blk:text-gray-400')} width={20} height={20} viewBox="0 0 20 20" fill="currentColor">
  ...
  let popupClassNames = 'absolute min-w-36 rounded-md bg-white shadow-xl blk:border blk:border-gray-700 blk:bg-gray-800';
  let panel;
