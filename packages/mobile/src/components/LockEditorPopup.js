...
const KBM_ACTIONS = [
  LOCK_ACTION_ADD_LOCK_NOTE, LOCK_ACTION_REMOVE_LOCK_NOTE, LOCK_ACTION_UNLOCK_NOTE,
...
const LockEditorPopup = () => {
  ...
  const onShowTitleInputChange = () => {
    setDoShowTitle(!doShowTitle);
  };
  ...
  if (lockAction === LOCK_ACTION_ADD_LOCK_NOTE) {
    ...
    desc = (
      <React.Fragment>
        <Text style={tailwind('mt-3 text-sm font-normal leading-6 text-gray-500 blk:text-gray-400')}>This note will be locked on this device only. If you forget your locked password, you can sign out to remove all locks.</Text>
        <Text style={tailwind('mt-3.5 text-sm font-normal leading-6 text-gray-500 blk:text-gray-400')}>Create a password for locking this note.</Text>
      </React.Fragment>
    );
    ...
    title = 'Unlock Note';
    desc = (
      <Text style={tailwind('mt-2 text-sm font-normal leading-6 text-gray-500 blk:text-gray-400')}>Enter your password to unlock this note.</Text>
    );
    btnText = 'Unlock';
  } else if (lockAction === LOCK_ACTION_ADD_LOCK_LIST) {
    title = 'Lock List';
