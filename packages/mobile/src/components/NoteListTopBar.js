...
const NoteListTopBar = (props) => {
  ...
  else title = <View style={tailwind('h-6 w-20 rounded-md bg-gray-300')} />;

  const menuBtnSvg = (
    <View style={tailwind('w-10 items-center justify-center rounded-full')}>
      <Svg width={24} height={24} style={tailwind('font-normal text-gray-500')} viewBox="0 0 20 20" fill="currentColor">
  ...
  if (syncProgress && syncProgress.status === SYNC) {
    ...
    innerMenuBtn = (
      <React.Fragment>
        ...
        <Animated.View style={[tailwind('absolute top-0 left-0 h-full justify-center'), innerMenuBtnStyle]}>
          <Svg width={40} height={40} style={tailwind('font-normal text-green-600')} viewBox="0 0 100 100" fill="none" stroke="currentColor" preserveAspectRatio="xMidYMid">
    ...
    innerMenuBtn = (
      <React.Fragment>
        {menuBtnSvg}
        <View style={tailwind('absolute top-1 right-2.5 h-1.5 w-1.5 rounded-full bg-red-500')} />
      </React.Fragment>
    );
    ...
    innerMenuBtn = (
      <React.Fragment>
        {menuBtnSvg}
        <View style={tailwind('absolute top-1 right-2.5 h-1.5 w-1.5 rounded-full bg-green-600')} />
      </React.Fragment>
    );
  } else {
  ...
  return (
    <View style={tailwind('flex-shrink-0 flex-grow-0')}>
      <View style={tailwind('h-16 flex-row items-center justify-between border-b border-gray-200')}>
        <TouchableOpacity onPress={onSidebarOpenBtnClick} style={tailwind('h-full justify-center border-r border-gray-200 px-4 sm:px-6 lg:hidden')}>
          <Svg width={24} height={24} style={tailwind('font-normal text-gray-500')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <View style={tailwind('flex-1 flex-row items-center justify-between pl-4 sm:pl-6')}>
          ...
          <View style={tailwind('ml-4 flex-row')}>
            <TouchableOpacity onPress={onSearchBtnClick} style={tailwind('items-center justify-center border border-white bg-white px-1 lg:hidden')}>
            ...
            <TouchableOpacity ref={menuBtn} onPress={onMenuBtnClick} style={tailwind('h-10 items-center justify-center border border-white bg-white pr-2')}>
              {innerMenuBtn}
            </TouchableOpacity>
          </View>
        </View>
