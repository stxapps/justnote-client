...
const SettingsPopupAccount = (props) => {
  ...
  if (userImageUrl) {
    userImage = (
      <FastImage style={tailwind('w-24 h-24 border-2 border-gray-200 rounded-full overflow-hidden')} source={{ uri: userImageUrl }} />
    );
  } else {
    userImage = (
        ...
        <View style={tailwind('mt-4 self-stretch md:flex-1 md:mt-0', safeAreaWidth)}>
          <View style={tailwind('flex-row items-start')}>
            ...
            <View style={tailwind('pl-2 flex-1')}>
              <Text style={tailwind('w-full text-base text-gray-500 font-normal leading-6.5')}>{username || 'N/A'}</Text>
            </View>
          </View>
          <View style={tailwind('flex-row items-start')}>
            <View style={{ width: profileFirstColWidth }}>
      ...
      <View style={tailwind('mt-4 p-4 rounded-md bg-yellow-50')}>
        <View style={tailwind('flex-row')}>
          <View style={tailwind('flex-shrink-0')}>
            <Svg style={tailwind('text-base text-yellow-400 font-normal')} width={warningSvgSize} height={warningSvgSize} viewBox="0 0 20 20" fill="currentColor">
              <Path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
            </Svg>
          </View>
          <View style={tailwind('ml-3 flex-1')}>
            <Text style={tailwind('text-base text-yellow-700 font-normal leading-6.5')}>Signing out from Justnote doesn’t sign out from Stacks. If you want to sign out from Stacks, especially when you use not-your-own devices, you need to go to <Text onPress={() => Linking.openURL('https://app.blockstack.org/')} style={tailwind('text-base text-yellow-700 font-normal underline')}>Stacks App</Text> and/or <Text onPress={() => Linking.openURL('https://browser.blockstack.org/account/delete')} style={tailwind('text-base text-yellow-700 font-normal underline')}>Blockstack Browser</Text> and sign out there.</Text>
          </View>
        </View>
      </View>
      <View style={tailwind('mt-8 mb-4')}>
        <Text style={tailwind('text-base text-red-600 font-medium leading-5')}>Delete Account</Text>
        <Text style={tailwind('mt-2.5 text-base text-gray-500 font-normal leading-6.5')}>Justnote uses Stacks Identity to sign you in. If you want to delete your Stacks Identity, please send an email to support@blockstack.com. For more information, please visit <Text onPress={() => Linking.openURL('https://forum.stacks.org/t/is-blockstack-gdrp-compliant/10931/4')} style={tailwind('text-base text-gray-500 font-normal underline')}>here</Text>.</Text>
      </View>
    </View>
  );
};
