...
const NoteEditor = (props) => {
  ...
  const style = useMemo(() => {
    if (Platform.OS === 'android') return tailwind('h-full w-full bg-white');
      ...
      return (
        <View style={tailwind('h-full w-full bg-white')}>
          <View style={[tailwind('items-center'), { paddingTop: 172 }]}>
            <View style={tailwind('h-32 w-32 items-center justify-center rounded-full bg-gray-200')}>
              <Svg width={64} height={64} style={tailwind('font-normal text-gray-500')} viewBox="0 0 60 60" fill="currentColor">
          ...
          <View style={tailwind('absolute inset-x-0 bottom-8')}>
            <Text style={tailwind('text-center text-sm font-normal text-gray-500')}>Click "+ New Note" button or select your note</Text>
          </View>
        </View>
      );
    }
