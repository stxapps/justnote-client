...
const ConfirmAsDummyPopup = () => {
  ...
  const isShown = useSelector(state => state.display.isConfirmAsDummyPopupShown);
  ...
  const onCancelBtnClick = useCallback(() => {
    ...
    dispatch(updatePopup(CONFIRM_AS_DUMMY_POPUP, false, null));
    didClick.current = true;
  }, [dispatch]);

  const onOkBtnClick = () => {
    if (didClick.current) return;
    dispatch(updateUserDummy(true));
    onCancelBtnClick();
    didClick.current = true;
  };
      ...
      <Animated.View style={[tailwind('w-full max-w-lg bg-white rounded-lg px-4 pt-5 pb-4 shadow-xl sm:my-8 sm:p-6', safeAreaWidth), popupStyle]}>
        <View style={tailwind('items-center sm:flex-row sm:items-start', safeAreaWidth)}>
          ...
          <View style={tailwind('flex-grow flex-shrink mt-3 sm:mt-0 sm:ml-4', safeAreaWidth)}>
            <Text style={tailwind('text-lg leading-6 font-medium text-gray-900 text-center sm:text-left', safeAreaWidth)}>Confirm continue?</Text>
            <View style={tailwind('mt-2')}>
              <Text style={tailwind('text-sm text-gray-500 font-normal text-center sm:text-left', safeAreaWidth)}>Without an account, your notes cannot be encrypted and synced to server. If you lose your device or accidentally uninstall the app, you'll lose your notes forever.</Text>
            </View>
          </View>
        </View>
        <View style={tailwind('mt-5 sm:mt-4 sm:ml-10 sm:pl-4 sm:flex-row', safeAreaWidth)}>
          <TouchableOpacity onPress={onOkBtnClick} style={tailwind('w-full rounded-md border border-red-600 shadow-sm px-4 py-2 bg-red-600 sm:w-auto', safeAreaWidth)}>
            <Text style={tailwind('text-base font-medium text-white text-center sm:text-sm', safeAreaWidth)}>I understand</Text>
          </TouchableOpacity>
...
export default React.memo(ConfirmAsDummyPopup);
