...
const SidebarListNames = () => {
  ...
  return (
    <View style={tailwind('flex-1 mt-6')}>
      <ScrollView>
        <View style={tailwind('-mt-1.5 pl-3 pr-1')}>
...
const _SidebarListName = (props) => {
  ...
  if (!isChildless) {
    if (listNameObj && listNameObj.children && listNameObj.children.length > 0) {
      ...
      expandBtn = (
        <TouchableOpacity onPress={onExpandBtnClick} style={tailwind('flex-grow-0 flex-shrink-0 justify-center items-center w-8 h-10 -ml-2.5 rounded lg:h-9', safeAreaWidth)}>
          <View style={tailwind('w-3.5 h-3.5 justify-center items-center rounded-sm ml-1.5')}>
            {expandSvg}
          </View>
        </TouchableOpacity>
      );
      ...
      expandBtn = (
        <View style={tailwind('flex-grow-0 flex-shrink-0 w-8 h-10 -ml-2.5 lg:h-9', safeAreaWidth)} />
      );
    }
  }
  ...
  return (
    <React.Fragment>
      <View style={[tailwind('flex-row justify-start items-center mt-1.5'), viewStyle]}>
        {expandBtn}
        <TouchableOpacity onPress={onListNameBtnClick} style={tailwind(`flex-grow flex-shrink min-w-0 ${btnClassNames} flex-row items-center px-2 py-2.5 rounded-md w-full lg:py-2`, safeAreaWidth)}>
          {svg}
