...
const NoteCommands = (props) => {
  ...
  const onMoreBtnClick = () => {
    const _rect = moreBtn.current.getBoundingClientRect();
    ...
    updatePopupUrlHash(BULK_EDIT_MENU_POPUP, true, rect);
    if (isFullScreen) onToggleFullScreen();
  };
  ...
  return (
    <React.Fragment>
      ...
      {isMoreBtnShown && <button ref={moreBtn} onClick={onMoreBtnClick} className={tailwind(`group inline-flex h-full items-center border pl-1 pr-1 text-sm focus:outline-none sm:pl-2 lg:rounded-md lg:px-2 lg:py-2 lg:shadow-sm ${btnClassNames}`)} type="button">
        <div className={tailwind(`rounded p-2 lg:p-0 lg:group-focus:bg-transparent ${btnInnerClassNames}`)}>
          <svg className={tailwind('h-5 w-5')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
