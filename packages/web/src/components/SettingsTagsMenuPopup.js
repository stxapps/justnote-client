...
import { moveTagName, updateTagNameEditors, updatePopupUrlHash } from '../actions';
import {
...
const SettingsTagsMenuPopup = () => {
  ...
  return (
    <AnimatePresence key="AP_stmPopup">
      <motion.button key="STMP_cancelBtn" ref={cancelBtn} onClick={onCancelBtnClick} className={tailwind('fixed inset-0 h-full w-full cursor-default bg-black bg-opacity-25 focus:outline-none')} variants={popupBgFMV} initial="hidden" animate="visible" exit="hidden" />
      {panel}
    </AnimatePresence>
  );
};
