...
const SettingsListsMenuPopup = () => {
  ...
  const buttons = (
    <div className="py-1">
      ...
      {![MY_NOTES, TRASH, ARCHIVE].includes(selectingListName) && <button onClick={onDeleteBtnClick} className="group w-full flex items-center px-4 py-3 text-sm text-left text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900" role="menuitem">
        <svg className="mr-3 w-4 text-gray-400 group-hover:text-gray-500 group-focus:text-gray-500" viewBox="0 0 14 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  let popupClassNames = 'fixed w-36 mt-1 rounded-md shadow-xl bg-white overflow-auto ring-1 ring-black ring-opacity-5';
  let panel;
  if (popupSize) {
  ...
  return (
    <AnimatePresence key="AP_slmPopup">
      <motion.button key="SLMP_cancelBtn" ref={cancelBtn} onClick={onCancelBtnClick} className="fixed inset-0 w-full h-full bg-black bg-opacity-25 cursor-default focus:outline-none" variants={popupBgFMV} initial="hidden" animate="visible" exit="hidden" />
      {panel}
    </AnimatePresence>
  );
};
