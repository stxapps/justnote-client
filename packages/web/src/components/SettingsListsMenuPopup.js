...
const SettingsListsMenuPopup = () => {
  ...
  const buttons = (
    <div className={tailwind('py-1')}>
      ...
      {![MY_NOTES, TRASH, ARCHIVE].includes(selectingListName) && <button onClick={onDeleteBtnClick} className={tailwind('group flex w-full items-center px-4 py-3 text-left text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:bg-gray-100 focus:text-gray-900 focus:outline-none')} role="menuitem">
        <svg className={tailwind('mr-3 w-4 text-gray-400 group-hover:text-gray-500 group-focus:text-gray-500')} viewBox="0 0 14 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  let popupClassNames = 'fixed w-36 overflow-auto rounded-md bg-white shadow-xl ring-1 ring-black ring-opacity-5';
  let panel;
  if (popupSize) {
  ...
  return (
    <AnimatePresence key="AP_slmPopup">
      <motion.button key="SLMP_cancelBtn" ref={cancelBtn} onClick={onCancelBtnClick} className={tailwind('fixed inset-0 h-full w-full cursor-default bg-black bg-opacity-25 focus:outline-none')} variants={popupBgFMV} initial="hidden" animate="visible" exit="hidden" />
      {panel}
    </AnimatePresence>
  );
};
