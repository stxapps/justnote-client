...
const SidebarListNames = () => {
  ...
  const buttons = listNameMap.map(listNameObj => {
    ...
    if (listNameObj.listName === TRASH) {
      svg = (
        <svg className={`${svgClassNames} mr-3 h-5 w-5`} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
