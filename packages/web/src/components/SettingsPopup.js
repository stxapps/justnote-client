...
import {
  ...
  SETTINGS_VIEW_TAGS, SETTINGS_VIEW_MISC, SETTINGS_VIEW_ABOUT, HASH_SUPPORT, LG_WIDTH,
  MERGING, DIED_MERGING,
} from '../types/const';
import {
    ...
    return (
      <div style={canvasStyle} className={tailwind('fixed inset-0 bg-white blk:bg-gray-900')}>
        <div className={tailwind('relative w-full h-full')}>
          ...
          {/* Sidebar for mobile */}
          <motion.div key="sidebar-for-mobile" className={tailwind('absolute inset-0 md:hidden')} variants={canvasFMV} initial={false} animate={animate}>
