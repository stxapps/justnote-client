...
const TopBarTitleQueryString = () => {
  ...
  const onCloseBtnClick = () => {
    dispatch(updateQueryString('', true));
  };
  ...
  return (
    <button onClick={onCloseBtnClick} className={tailwind('group flex max-w-full items-center rounded-sm focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 blk:focus:ring-gray-500 blk:focus:ring-offset-gray-900')}>
      <h1 className={tailwind('mr-1 truncate text-lg font-medium leading-6 text-gray-900 blk:text-gray-100')}>{`#${displayName}`}</h1>
      <svg className={tailwind('mt-px h-5 w-5 flex-shrink-0 flex-grow-0 rounded-full text-gray-400 group-hover:text-gray-500 blk:text-gray-400 blk:group-hover:text-gray-300')} viewBox="0 0 28 28" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
