...
const ListNamesPopup = () => {
          ...
          return (
            <div key={obj.listName} className={tailwind('flex w-full flex-row items-center justify-start')}>
              ...
              {(obj.children && obj.children.length > 0) && <button onClick={() => onForwardBtnClick(obj.listName)} className={tailwind('group flex h-10 w-10 flex-shrink-0 flex-grow-0 items-center justify-center focus:outline-none')} disabled={forwardDisabled}>
                <svg className={tailwind(`h-5 w-5 rounded ${forwardDisabled ? 'text-gray-300' : 'text-gray-500 group-hover:text-gray-700 group-focus:ring-2 group-focus:ring-offset-0 group-focus:ring-gray-400'}`)} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      ...
      <div className={tailwind('flex h-full w-full flex-col')}>
        <div className={tailwind('flex h-11 w-full flex-shrink-0 flex-grow-0 items-center justify-start pt-1')}>
          {currentListName && <button onClick={() => onBackBtnClick(parent)} className={tailwind('group h-10 flex-shrink-0 flex-grow-0 items-center justify-center pl-2.5 pr-1 focus:outline-none')}>
            <svg className={tailwind('h-5 w-5 rounded text-gray-500 group-hover:text-gray-700 group-focus:ring-2 group-focus:ring-gray-400 group-focus:ring-offset-0')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        <div className={tailwind('flex w-full flex-shrink-0 flex-grow-0 flex-row items-center justify-end border-t border-gray-200 px-3 py-2.5')}>
          <button onClick={onMoveHereBtnClick} className={tailwind(`rounded-full border bg-white px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 ${moveHereDisabled ? 'border-gray-300' : 'border-gray-400 hover:border-gray-500'} ${moveHereDisabled ? 'text-gray-400' : 'text-gray-500 hover:text-gray-600'}`)} disabled={moveHereDisabled}>
  ...
  const panel = (
    <motion.div key="LNP_popup" style={popupStyle} className={tailwind(`fixed overflow-auto rounded-md bg-white shadow-xl ring-1 ring-black ring-opacity-5 ${popupClassNames}`)} variants={popupFMV} initial="hidden" animate="visible" exit="hidden" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
      {_render()}
    </motion.div>
  );
