...
const ListNamesPopup = () => {
          ...
          return (
            <div key={obj.listName} className="w-full flex flex-row justify-start items-center">
              ...
              {(obj.children && obj.children.length > 0) && <button onClick={() => onForwardBtnClick(obj.listName)} className="flex-grow-0 flex-shrink-0 w-10 h-10 flex justify-center items-center group focus:outline-none" disabled={forwardDisabled}>
                <svg className={`w-5 h-5 ${forwardDisabled ? 'text-gray-300' : 'text-gray-500 group-hover:text-gray-700 group-focus:ring-2 group-focus:ring-offset-0 group-focus:ring-gray-400'} rounded`} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  const _render = () => {
    ...
    return (
      <div className="w-full h-full flex flex-col">
        <div className="flex-grow-0 flex-shrink-0 flex justify-start items-center w-full h-11 pt-1">
          {currentListName && <button onClick={() => onBackBtnClick(parent)} className="pl-2.5 pr-1 h-10 flex-grow-0 flex-shrink-0 justify-center items-center group focus:outline-none">
            <svg className="w-5 h-5 text-gray-500 rounded group-hover:text-gray-700 group-focus:ring-2 group-focus:ring-offset-0 group-focus:ring-gray-400" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        ...
        <div className="flex-grow-0 flex-shrink-0 w-full px-3 py-2.5 border-t border-gray-200 flex flex-row justify-end items-center">
          <button onClick={onMoveHereBtnClick} className={`px-3 py-1.5 bg-white border ${moveHereDisabled ? 'border-gray-300' : 'border-gray-400 hover:border-gray-500'} rounded-full text-xs ${moveHereDisabled ? 'text-gray-400' : 'text-gray-500 hover:text-gray-600'} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400`} disabled={moveHereDisabled}>
  ...
  return (
    <AnimatePresence key="AP_lnPopup">
      <motion.button key="LNP_cancelBtn" ref={cancelBtn} onClick={onCancelBtnClick} className="fixed inset-0 w-full h-full bg-black bg-opacity-25 cursor-default focus:outline-none" variants={popupBgFMV} initial="hidden" animate="visible" exit="hidden" />
      {panel}
    </AnimatePresence>
  );
};
