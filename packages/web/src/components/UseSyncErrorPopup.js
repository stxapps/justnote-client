...
import { HASH_SUPPORT, USE_SYNC_ERROR_POPUP } from '../types/const';
...
const UseSyncErrorPopup = () => {
  ...
  const onDisableBtnClick = () => {
    ...
    dispatch(disableSyncMode());
    didClick.current = true;
  };

  const onCloseBtnClick = () => {
    if (didClick.current) return;
    dispatch(updatePopup(USE_SYNC_ERROR_POPUP, false));
    didClick.current = true;
  };
      ...
      <div className={tailwind('relative m-4 rounded-md bg-red-50 p-4 shadow-lg')}>
        <div className={tailwind('flex')}>
          ...
          <div className={tailwind('ml-3 lg:mt-0.5')}>
            ...
            <p className={tailwind('mt-2.5 text-sm text-red-700')}>Please check if IndexedDB is supported and reload the page (e.g., it's disabled by default in private browsing mode). Or disable the sync mode. <br className={tailwind('hidden sm:inline')} />If the problem persists, please <a className={tailwind('rounded underline hover:text-red-800 focus:outline-none focus:ring-2 focus:ring-red-700')} href={'/' + HASH_SUPPORT} target="_blank" rel="noreferrer">contact us</a>.</p>
            <div className={tailwind('mt-4')}>
              <div className={tailwind('-mx-2 -my-1.5 flex')}>
                <button onClick={onDisableBtnClick} className={tailwind('rounded-md bg-red-50 px-2 py-1.5 text-sm font-medium text-red-800 transition duration-150 ease-in-out hover:bg-red-100 focus:bg-red-100 focus:outline-none')}>Disable</button>
              </div>
            </div>
          </div>
        </div>
...
export default React.memo(UseSyncErrorPopup);
