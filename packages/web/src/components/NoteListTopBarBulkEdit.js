...
const NoteListTopBarBulkEdit = () => {
  ...
  return (
    <div className={tailwind('h-16 w-full flex-shrink-0 flex-grow-0 border-b border-gray-200')}>
      <div className={tailwind('flex h-full w-full justify-between sm:pl-3 sm:pr-6')}>
        <button onClick={onBulkEditCancelBtnClick} className={tailwind('group h-full px-2 text-gray-500 hover:text-gray-700 focus:text-gray-700 focus:outline-none')}>
          <div className={tailwind('flex items-center justify-center rounded px-2 py-2 group-focus:bg-gray-200')}>
        ...
        <div className={tailwind('flex h-full flex-1 items-center justify-between sm:pl-2')}>
          <div className={tailwind('min-w-0 flex-shrink flex-grow')}>
            <h1 className={tailwind('truncate text-lg font-medium leading-6 text-gray-900')}>{selectedNoteIdsLength} Selected</h1>
          </div>
