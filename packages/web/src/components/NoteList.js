...
const NoteList = (props) => {
          ...
          <div className="m-4 p-4 bg-red-50 rounded-md shadow-lg">
            <div className="flex">
              ...
              <div className="ml-3 mt-0.5">
                <h3 className="text-sm text-red-800 text-left leading-5">To prevent network overload, up to {MAX_SELECTED_NOTE_IDS} items can be selected.</h3>
              </div>
            </div>
          </div>
        </motion.div>
  ...
  return (
    <div className="relative w-full min-w-64 h-full flex flex-col">
      ...
      {noteListItems}
      {/* Add button */}
      {!isBulkEditing && (listName !== TRASH) && <button onClick={onAddBtnClick} className="absolute right-4 bottom-4 rounded-full bg-green-600 text-white w-16 h-16 shadow-md flex items-center justify-center hover:bg-green-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-600 lg:hidden">
