...
const NoteListTopBar = (props) => {
  ...
  let title;
  if (didFetch) title = <h1 className={tailwind('truncate text-lg font-medium leading-6 text-gray-900')}>{getListNameDisplayName(listName, listNameMap)}</h1>;
  else title = <div className={tailwind('h-6 w-20 animate-pulse rounded-md bg-gray-300')} />;

  const menuBtnSvg = (
    <svg className={tailwind('w-10 rounded-full p-2 group-focus:bg-gray-200')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className={tailwind('flex-shrink-0 flex-grow-0')}>
      <div className={tailwind('flex h-16 items-center justify-between border-b border-gray-200')}>
        <button onClick={onSidebarOpenBtnClick} className={tailwind('h-full border-r border-gray-200 px-4 text-gray-500 hover:text-gray-700 focus:bg-gray-200 focus:text-gray-700 focus:outline-none sm:px-6 lg:hidden')}>
        ...
        <div className={tailwind('flex min-w-0 flex-1 items-center justify-between pl-4 sm:pl-6')}>
          ...
          <div className={tailwind('ml-4 flex')}>
            <button onClick={onSearchBtnClick} type="button" className={tailwind('group inline-flex items-center border border-white bg-white px-1 text-sm text-gray-500 hover:text-gray-700 focus:text-gray-700 focus:outline-none lg:hidden')}>
              <div className={tailwind('rounded p-2.5 group-focus:bg-gray-200')}>
            ...
            <button ref={menuBtn} onClick={onMenuBtnClick} type="button" className={tailwind('group relative inline-flex items-center border border-white bg-white pr-2 text-sm text-gray-500 hover:text-gray-700 focus:text-gray-700 focus:outline-none')}>
              {innerMenuBtn}
            </button>
          </div>
        </div>
