    ...
    <div className={tailwind('group flex items-center rounded-sm py-4 pl-3 sm:pl-5')}>
      ...
      <div className={tailwind('min-w-0 flex-1')}>
        <div className={tailwind('pr-3')}>
          <button tabIndex={titleTabIndex} onTouchStart={onTouchPress} onTouchMove={onTouchCancel} onTouchEnd={onTouchPressRelease} onTouchCancel={onTouchCancel} onMouseDown={onClickPress} onMouseUp={onClickPressRelease} onMouseLeave={onClickCancel} className={tailwind(`flex w-full items-center justify-between rounded-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-400 focus-visible:ring-offset-2 ${titleClassNames}`)}>
            <h3 className={tailwind('min-w-0 flex-1 truncate text-left text-base font-semibold text-gray-800 group-hover:underline lg:text-sm')}>{note.title}</h3>
            <p className={tailwind('ml-3 flex-shrink-0 flex-grow-0 whitespace-nowrap text-left text-xs text-gray-400')}>{noteDate}</p>
          </button>
        </div>
        <div className={tailwind(`flex items-center justify-between ${isBulkEditing ? 'pr-3' : ''}`)}>
          <button tabIndex={bodyTabIndex} onTouchStart={onTouchPress} onTouchMove={onTouchCancel} onTouchEnd={onTouchPressRelease} onTouchCancel={onTouchCancel} onMouseDown={onClickPress} onMouseUp={onClickPressRelease} onMouseLeave={onClickCancel} className={tailwind('min-h-[2.625rem] w-full min-w-0 flex-1 rounded-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-400 focus-visible:ring-offset-2')}>
            <p ref={pBodyRef} className={tailwind('text-left text-sm text-gray-500 line-clamp-3')}>{body}</p>
          </button>
          {!isBulkEditing && <button onClick={onMenuBtnClick} className={tailwind('flex-shrink-0 flex-grow-0 py-1 pl-4 pr-2 text-gray-400 focus:outline-none group-s')} disabled={isBusy}>
            <svg className={tailwind('w-[1.125rem] rounded-full py-2 group-s-hover:bg-gray-200 group-s-focus-visible:bg-gray-200')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
