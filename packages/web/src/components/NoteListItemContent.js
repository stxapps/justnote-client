      ...
      <div className={tailwind(`pl-3 sm:pl-5 ${tagClassNames}`)}>
        <div className={tailwind('flex flex-wrap items-center justify-start pt-1')}>
          {tnAndDns.map((tnAndDn, i) => {
            return (
              <button key={tnAndDn.tagName} onClick={() => dispatch(updateQueryString(tnAndDn.tagName))} className={tailwind(`group mt-2 block rounded-md bg-gray-100 px-2 py-1 hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 blk:bg-gray-700 blk:hover:bg-gray-600 blk:focus:ring-gray-500 blk:focus:ring-offset-gray-900 ${i === 0 ? '' : 'ml-2'}`)}>
