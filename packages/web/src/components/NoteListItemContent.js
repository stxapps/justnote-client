      ...
      <div className={tailwind('min-w-0 flex-1')}>
        <div className={tailwind('pr-3')}>
          <button tabIndex={titleTabIndex} onTouchStart={onTouchPress} onTouchMove={onTouchCancel} onTouchEnd={onTouchPressRelease} onTouchCancel={onTouchCancel} onMouseDown={onClickPress} onMouseUp={onClickPressRelease} onMouseLeave={onClickCancel} className={tailwind(`flex w-full items-center justify-between rounded-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-400 focus-visible:ring-offset-2 ${titleClassNames}`)}>
            <h3 className={tailwind('min-w-0 flex-1 truncate text-left text-base font-semibold text-gray-800 group-hover:underline blk:text-gray-100 blk:group-hover:text-white lg:text-sm')}>{note.title}</h3>
        ...
        <div className={tailwind(`flex items-center justify-between ${isBulkEditing ? 'pr-3' : ''}`)}>
          ...
          {!isBulkEditing && <button onClick={onMenuBtnClick} className={tailwind('flex-shrink-0 flex-grow-0 py-1 pl-4 pr-2 text-gray-400 focus:outline-none group-s blk:text-gray-500')} disabled={isBusy}>
            <svg className={tailwind('w-[1.125rem] rounded-full py-2 group-s-hover:bg-gray-200 group-s-focus-visible:bg-gray-200 blk:group-s-hover:bg-gray-700 blk:group-s-focus-visible:bg-gray-700')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
