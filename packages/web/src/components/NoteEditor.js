...
const NoteEditor = (props) => {
  ...
  if (!note) {
    return (
      <div className={tailwind('relative h-full w-full bg-white')}>
        <div style={{ top: '172px' }} className={tailwind('absolute inset-x-0')}>
          <div className={tailwind('mx-auto flex h-32 w-32 items-center justify-center rounded-full bg-gray-200')}>
            <svg className={tailwind('h-16 w-16 text-gray-500')} viewBox="0 0 60 60" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className={tailwind('absolute inset-x-0 bottom-8')}>
          <p className={tailwind('text-center text-sm text-gray-500')}>Click "+ New Note" button or select your note</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={tailwind('flex h-full w-full flex-col bg-white')}>
