      ...
      <div className={tailwind('fixed inset-0 overflow-y-auto')} aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div style={{ minHeight: safeAreaHeight }} className={tailwind('flex items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0')}>
          ...
          <motion.div className={tailwind('relative inline-block overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left align-bottom shadow-xl sm:my-8 sm:w-full sm:max-w-lg sm:p-6 sm:align-middle')} variants={dialogFMV} initial="hidden" animate="visible" exit="hidden">
            ...
            <div className={tailwind('mt-5 sm:mt-6 sm:flex sm:items-center sm:justify-between')}>
              <button onClick={onOkBtnClick} className={tailwind('inline-flex w-full justify-center rounded-md border border-gray-800 bg-gray-800 py-2 text-base font-medium text-white shadow-sm hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-600 focus:ring-offset-2 sm:mr-1.5 sm:flex-shrink sm:flex-grow sm:text-sm')} type="button">More info</button>
              <button onClick={onCancelBtnClick} style={cancelBtnStyle} className={tailwind('mt-3 inline-flex w-full justify-center rounded-md border border-gray-400 bg-white text-base font-normal text-gray-500 shadow-sm hover:border-gray-500 hover:text-gray-600 focus:border-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 sm:mt-0 sm:ml-1.5 sm:flex-shrink sm:flex-grow sm:text-sm')} type="button">Cancel</button>
            </div>
          </motion.div>
        </div>
      </div>
