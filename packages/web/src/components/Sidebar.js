...
const Sidebar = () => {
  ...
  const derivedUserImage = useMemo(() => {
    if (userImage) {
      return (
        <img className={tailwind('h-10 w-10 flex-shrink-0 rounded-full bg-gray-300')} src={userImage} alt="Profile" />
      );
    }

    return (
      <svg className={tailwind('h-10 w-10 flex-shrink-0 text-gray-200 group-hover:text-gray-300')} viewBox="0 0 96 96" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className={tailwind('flex h-full w-full min-w-56 flex-col bg-gray-100 pt-5 pb-4')}>
      <div className={tailwind('flex flex-shrink-0 items-center px-6')}>
        <img className={tailwind('h-8 w-auto')} src={logoFull} alt="Justnote" />
      </div>
      <div className={tailwind('flex h-0 flex-1 flex-col')}>
        ...
        <div className={tailwind('relative mt-6 hidden pl-3 pr-1 text-left')}>
          <button ref={profileBtn} onClick={onProfileBtnClick} type="button" className={tailwind('group w-full rounded-md bg-gray-100 px-3.5 py-2 text-sm font-medium text-gray-700 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none')} aria-haspopup="true" aria-expanded="true">
            <span className={tailwind('flex w-full items-center justify-between')}>
              <span className={tailwind('flex min-w-0 flex-1 items-center justify-between space-x-3')}>
                {derivedUserImage}
                <span className={tailwind('min-w-0 flex-1')}>
                  <span className={tailwind('truncate text-sm font-medium text-gray-900')}>{derivedUsername}</span>
                </span>
              </span>
              <svg className={tailwind('h-5 w-5 flex-shrink-0 text-gray-400 group-hover:text-gray-500 group-focus:text-gray-500')} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        ...
        <div className={tailwind('mt-6 hidden pl-3 pr-1 lg:block')}>
          <button onClick={onAddBtnClick} type="button" className={tailwind('w-full rounded-md border border-green-600 bg-green-600 py-2 text-sm font-medium text-white shadow-sm hover:bg-green-500 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 focus:ring-offset-gray-100')}>
