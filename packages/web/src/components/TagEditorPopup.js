...
import {
  updatePopupUrlHash, updateTagEditor, addTagEditorTagName, updateTagData,
} from '../actions';
import {
    ...
    return (
      <AnimatePresence key="AnimatePresence_TEP">
        <div className={tailwind('fixed inset-0 overflow-hidden')}>
          <div className={tailwind('flex items-start justify-center p-4')} style={{ minHeight: safeAreaHeight }}>
    ...
    <AnimatePresence key="AnimatePresence_TEP">
      <div className={tailwind('fixed inset-0 overflow-hidden')}>
        <div className={tailwind('flex items-center justify-center p-4')} style={{ minHeight: safeAreaHeight }}>
          ...
          <motion.div className={tailwind('w-full max-w-sm overflow-hidden rounded-lg bg-white shadow-xl blk:bg-gray-800 blk:ring-1 blk:ring-white blk:ring-opacity-25 lg:mb-20')} variants={dialogFMV} initial="hidden" animate="visible" exit="hidden" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <div className={tailwind('relative flex flex-col overflow-hidden rounded-lg bg-white blk:bg-gray-800')} style={{ maxHeight: panelHeight }}>
              <div className={tailwind('relative flex-1 overflow-y-auto overflow-x-hidden px-4 pt-8 pb-4 sm:px-6 sm:pb-6')}>
                    ...
                    return (
                      <div key={`TagEditorValue-${value.tagName}`} className={tailwind(`mb-2 flex max-w-full items-center justify-start rounded-full bg-gray-100 pl-3 blk:bg-gray-700 ${i === 0 ? '' : 'ml-2'}`)}>
                        ...
                        <button onClick={() => onValueDeselect(value)} className={tailwind('group ml-1 flex-shrink-0 flex-grow-0 items-center justify-center rounded-full py-1.5 pr-1.5 focus:outline-none')} type="button">
                          <svg className={tailwind('h-5 w-5 cursor-pointer rounded-full text-gray-400 group-hover:text-gray-500 group-focus:ring-2 group-focus:ring-gray-400 group-focus:ring-offset-0 blk:text-gray-400 blk:group-hover:text-gray-300 blk:group-focus:ring-gray-500')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className={tailwind(`flex items-center justify-start ${tagEditor.msg ? '' : 'pt-5'}`)}>
                  ...
                  <input onChange={onDnInputChange} onKeyDown={onDnInputKeyPress} className={tailwind('block w-full flex-1 rounded-md border border-gray-400 bg-white px-3.5 py-[0.3125rem] text-sm text-gray-700 placeholder:text-gray-500 focus:border-gray-500 focus:outline-none focus:ring-gray-500 blk:border-gray-500 blk:bg-gray-800 blk:text-gray-200 blk:placeholder:text-gray-400 blk:focus:border-gray-400 blk:focus:ring-gray-400')} placeholder="Add a new tag" value={tagEditor.displayName} id="new-tag-input" name="new-tag-input" type="text" />
                ...
                {tagEditor.hints.length > 0 && <div className={tailwind('flex flex-wrap items-center justify-start pt-4')}>
                  ...
                  {tagEditor.hints.map(hint => {
                    return (
                      <button key={`TagEditorHint-${hint.tagName}`} onClick={() => onHintSelect(hint)} className={tailwind(`group ml-2 mb-2 block max-w-full rounded-full bg-gray-100 px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-0 blk:bg-gray-700 blk:focus:ring-gray-500 blk:focus:ring-offset-2 blk:focus:ring-offset-gray-800 ${hint.isBlur ? '' : 'hover:bg-gray-200 blk:hover:bg-gray-600'}`)} type="button" disabled={hint.isBlur}>
                ...
                <div className={tailwind('mt-6 border-t border-gray-200 pt-3.5 blk:border-gray-600 sm:pt-5')}>
                  <button onClick={onSaveBtnClick} style={{ paddingTop: '0.4375rem', paddingBottom: '0.4375rem' }} className={tailwind('rounded-md bg-gray-800 px-4 text-sm font-medium text-gray-50 hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 blk:bg-gray-100 blk:text-gray-800 blk:hover:bg-white blk:focus:ring-gray-500 blk:focus:ring-offset-gray-800')} type="button">Save</button>
                  <button onClick={onPopupCloseBtnClick} className={tailwind('ml-2 rounded-md px-2.5 py-1.5 text-sm text-gray-500 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-0 blk:text-gray-300 blk:hover:bg-gray-700 blk:focus:ring-gray-500')} type="button">Cancel</button>
                </div>
              </div>
            </div>
          </motion.div>
