...
const NoteListItem = (props) => {
  ...
  const renderBusy = () => {
    ...
    return (
      <div className="absolute top-0 right-0 w-12 h-12 bg-transparent overflow-hidden">
        <div className="relative w-12 h-12 bg-green-600 overflow-hidden transform rotate-45 translate-x-1/2 -translate-y-1/2">
          <svg style={svgStyle} className="relative w-5 h-5 text-green-100 transform -rotate-45 -translate-x-1/2 -translate-y-full" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <li className="relative px-4 py-5 sm:px-6">
      {content}
      {[ADDING, MOVING].includes(note.status) && renderBusy()}
