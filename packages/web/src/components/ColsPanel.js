...
const ColsPanel = () => {
  ...
  return (
    <div style={{ height: safeAreaHeight }} className={tailwind('flex w-full overflow-hidden bg-white')}>
      <motion.div style={{ width: state.pane1Width }} className={tailwind(`overflow-hidden bg-white ${pane1Classes}`)} layout={!isResizeActive}>
        <Sidebar />
      </motion.div>
      <motion.div onMouseDown={onLeftResizerMouseDown} onTouchStart={onLeftResizerTouchStart} onTouchEnd={onTouchEnd} className={tailwind(`relative w-2 cursor-resize overflow-visible bg-gray-100 ${resizer1Classes}`)} layout={!isResizeActive}>
        <button onClick={onTogglePane1Shown} className={tailwind('group absolute right-0 bottom-8 h-7 w-5 rounded-tl rounded-bl bg-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-400')}>
      ...
      <motion.div style={{ width: state.pane2Width }} className={tailwind(`relative overflow-hidden bg-white ${pane2Classes}`)} layout={!isResizeActive}>
        <NoteList />
        <AnimatePresence>
          {!state.isPane1Shown && <motion.button onClick={onTogglePane1Shown} className={tailwind('group absolute left-0 bottom-8 h-7 w-5 rounded-tr rounded-br bg-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-400')} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <svg className={tailwind('h-5 w-5 text-gray-400 group-hover:text-gray-500')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      <motion.div onMouseDown={onRightResizerMouseDown} onTouchStart={onRightResizerTouchStart} onTouchEnd={onTouchEnd} className={tailwind(`relative cursor-resize overflow-visible border-l border-gray-100 bg-white pr-1 ${resizer2Classes}`)} layout={!isResizeActive} />
      <motion.div className={tailwind('flex-1 overflow-hidden bg-white')} layout={!isResizeActive} onLayoutAnimationComplete={onPane3AnimEnd}>
