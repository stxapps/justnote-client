...
const ColsPanel = () => {
  ...
  return (
    <div style={{ height: safeAreaHeight }} className={tailwind('flex w-full overflow-hidden bg-white blk:bg-gray-900')}>
      <motion.div style={{ width: state.pane1Width }} className={tailwind(`overflow-hidden bg-white blk:bg-gray-900 ${pane1Classes}`)} layout={!isResizeActive}>
        <Sidebar />
      </motion.div>
      <motion.div onMouseDown={onLeftResizerMouseDown} onTouchStart={onLeftResizerTouchStart} onTouchEnd={onTouchEnd} className={tailwind(`relative w-2 cursor-resize overflow-visible bg-gray-100 blk:bg-gray-800 ${resizer1Classes}`)} layout={!isResizeActive}>
      ...
      <motion.div style={{ width: state.pane2Width }} className={tailwind(`relative overflow-hidden bg-white blk:bg-gray-900 ${pane2Classes}`)} layout={!isResizeActive}>
        <NoteList />
        <AnimatePresence>
          {!state.isPane1Shown && <motion.button onClick={onTogglePane1Shown} className={tailwind('group absolute left-0 bottom-8 h-7 w-5 rounded-tr rounded-br bg-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-400 blk:bg-gray-600 blk:focus:ring-gray-500')} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <svg className={tailwind('h-5 w-5 text-gray-400 group-hover:text-gray-500 blk:text-gray-500 blk:group-hover:text-gray-400')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      <motion.div onMouseDown={onRightResizerMouseDown} onTouchStart={onRightResizerTouchStart} onTouchEnd={onTouchEnd} className={tailwind(`relative cursor-resize overflow-visible border-l border-gray-100 bg-white pr-1 blk:border-gray-800 blk:bg-gray-900 ${resizer2Classes}`)} layout={!isResizeActive} />
      <motion.div className={tailwind('flex-1 overflow-hidden bg-white blk:bg-gray-900')} layout={!isResizeActive} onLayoutAnimationComplete={onPane3AnimEnd}>
