...
const NoteEditorConflict = (props) => {
  ...
  const renderLoading = () => {
    ...
    return (
      <React.Fragment>
        <div className={tailwind('absolute inset-0 bg-white bg-opacity-25')} />
          ...
          <div className={tailwind('m-4 rounded-md bg-red-50 p-4 shadow-lg')}>
            <div className={tailwind('flex')}>
              ...
              <div className={tailwind('ml-3 lg:mt-0.5')}>
                ...
                <p className={tailwind('mt-2.5 text-sm text-red-700')}>Please wait a moment and try again.<br />If the problem persists, please <a className={tailwind('rounded-sm underline hover:text-red-800 focus:outline-none focus:ring-2 focus:ring-red-700')} href={'/' + HASH_SUPPORT} target="_blank" rel="noreferrer">contact us
  ...
  return (
    <div className={tailwind('relative h-full w-full overflow-auto bg-white')}>
      <div style={style} className={tailwind('relative px-4 pb-4 sm:px-6 sm:pb-6')}>
        <div className={tailwind('h-16 w-full')} />
        <h3 className={tailwind('pt-5 text-lg font-medium text-gray-800')}>{conflictedNote.notes.length} Versions found</h3>
        <p className={tailwind('text-sm font-normal text-gray-500')}>Please choose the correct version of this note.</p>
        <AnimateSharedLayout>
        ...
        <div className={tailwind('absolute top-0 left-0 lg:hidden')}>
          <button onClick={onRightPanelCloseBtnClick} type="button" className={tailwind('bg-white px-4 py-4 text-sm text-gray-500 hover:text-gray-700 focus:bg-gray-200 focus:text-gray-700 focus:outline-none')}>
...
const _ConflictItem = (props) => {
  ...
  return (
    <motion.div className={tailwind('mt-6 rounded-lg border border-gray-200')} layout={true}>
      <motion.div className={tailwind(`rounded-t-lg bg-gray-50 sm:flex sm:items-start sm:justify-between ${!isOpen ? 'rounded-b-lg' : ''}`)} layout={true}>
        <div className={tailwind('sm:flex-shrink sm:flex-grow')}>
          <button onClick={onOpenBtnClick} type="button" className={tailwind('group flex w-full rounded-lg pt-3 pl-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-gray-400')}>
            {arrowSvg}
            <div className={tailwind('ml-1')}>
              <div className={tailwind('text-left text-base font-medium text-gray-800 group-hover:underline lg:text-sm')}>Last update on {updatedDTStr}</div>
              <div className={tailwind('mt-1 text-left text-sm text-gray-600')}>In {getListNameDisplayName(listName, listNameMap)}</div>
            </div>
          </button>
        </div>
      ...
      <AnimatePresence>
        {isOpen && <motion.div className={tailwind('px-4 py-5')} variants={slideYFMV} initial="hidden" animate="visible" exit="hidden">
          <h3 className={tailwind('text-lg font-medium text-gray-800')}>{note.title}</h3>
          <p className={tailwind('mt-3 text-base font-normal text-gray-600')}>{note.body}</p>
        </motion.div>}
      </AnimatePresence>
    </motion.div>
  );
