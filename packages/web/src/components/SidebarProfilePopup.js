...
const SidebarProfilePopup = () => {
  ...
  const onProfileCancelBtnClick = () => {
    updatePopupUrlHash(PROFILE_POPUP, false, null);
  };
  ...
  return (
    <AnimatePresence key="AP_Sb_ProfilePopup">
      <motion.button key="Sb_ProfilePopup_cancelBtn" ref={cancelBtn} onClick={onProfileCancelBtnClick} className={tailwind('fixed inset-0 h-full w-full cursor-default bg-black bg-opacity-25 focus:outline-none')} variants={popupBgFMV} initial="hidden" animate="visible" exit="hidden" />
      <motion.div key="Sb_ProfilePopup_popup" style={popupStyle} className={tailwind('absolute origin-top rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5')} variants={popupFMV} initial="hidden" animate="visible" exit="hidden" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
