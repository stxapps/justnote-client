...
const SidebarProfilePopup = () => {
  ...
  return (
    <AnimatePresence key="AP_Sb_ProfilePopup">
      <motion.button key="Sb_ProfilePopup_cancelBtn" onClick={onProfileCancelBtnClick} tabIndex={-1} className="fixed inset-0 w-full h-full bg-black opacity-25 cursor-default focus:outline-none" variants={popupBgFMV} initial="hidden" animate="visible" exit="hidden"></motion.button>
      <motion.div key="Sb_ProfilePopup_menuPopup" style={menuPopupStyle} className="absolute origin-top mt-1 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5" variants={popupFMV} initial="hidden" animate="visible" exit="hidden" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
        <div className="py-1">
          <button className="group w-full flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" role="menuitem">
