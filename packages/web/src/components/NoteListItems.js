    ...
    if (listName === MY_NOTES) {
      return (
        <div className="w-full h-full relative">
          <div style={{ top: '128px' }} className="absolute inset-x-0 px-4 sm:px-6 lg:px-8">
            ...
            <p className="mt-6 text-sm font-semibold text-gray-800 text-center tracking-wide">No notes in {displayName}</p>
            <p style={{ lineHeight: '22px' }} className="mt-4 text-sm text-gray-600 text-center tracking-wide">Click <span className="font-semibold">"+ New Note"</span> button to add a new note</p>
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="w-full h-full relative">
          <div style={{ top: '128px' }} className="absolute inset-x-0 px-4 sm:px-6 lg:px-8">
            ...
            <p className="mt-6 text-sm font-semibold text-gray-800 text-center tracking-wide">No notes in {displayName}</p>
            <p style={{ lineHeight: '22px' }} className="mt-4 text-sm text-gray-600 text-center tracking-wide">Click <span className="font-semibold">"Remove"</span> from the menu to move notes you don't need anymore here</p>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="w-full h-full relative">
        <div style={{ top: '128px' }} className="absolute inset-x-0 px-4 sm:px-6 lg:px-8">
          ...
          <p className="mt-6 text-sm font-semibold text-gray-800 text-center tracking-wide">No notes in {displayName}</p>
          <p style={{ lineHeight: '22px' }} className="mt-4 text-sm text-gray-600 text-center tracking-wide">Click <span className="font-semibold">{textName}</span> from the menu to move notes here</p>
        </div>
      </div>
    );
  };
  ...
  const renderFetchMoreBtn = () => {
    return (
      <div className="my-6 px-4 sm:px-6 lg:px-8">
        <button onClick={onFetchMoreBtnClick} className="w-full flex justify-center items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-600 bg-white hover:bg-gray-50">More</button>
      </div>
    );
  };
