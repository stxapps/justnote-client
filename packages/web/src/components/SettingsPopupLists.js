...
const _ListNameEditor = (props) => {
  ...
  if (isBusy) {
    ...
    if (listNameObj && [MY_NOTES, TRASH, ARCHIVE].includes(listNameObj.listName)) {
      ...
      deleteBtn = (
        <button onClick={onDeleteBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-start items-center w-8 h-10 group focus:outline-none">
          <svg className="h-4 text-gray-500 rounded-sm group-hover:text-gray-600 group-focus:ring-2 group-focus:ring-offset-4 group-focus:ring-gray-400" viewBox="0 0 14 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className="mt-1 flex justify-start items-center">
      {(state.mode === MODE_VIEW && listNameObj === null) && <button onClick={onAddBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-start items-center w-8 h-10 group focus:outline-none">
        <svg style={{ width: '0.875rem', height: '0.875rem' }} className="text-gray-500 rounded-sm group-hover:text-gray-600 group-focus:ring-2 group-focus:ring-offset-4 group-focus:ring-gray-400" viewBox="0 0 14 14" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      {state.mode === MODE_EDIT && <button onTouchStart={onCancelBtnPress} onMouseDown={onCancelBtnPress} onClick={onCancelBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-start items-center w-8 h-10 group focus:outline-none">
        <svg className="w-3 h-3 text-gray-500 rounded-sm group-hover:text-gray-600 group-focus:ring-2 group-focus:ring-offset-4 group-focus:ring-gray-400" viewBox="0 0 12 12" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="relative flex-grow flex-shrink">
        <input ref={input} onFocus={onInputFocus} onBlur={onInputBlur} onChange={onInputChange} onKeyPress={onInputKeyPress} className="px-0 py-2 w-full bg-white text-base text-gray-600 border-0 focus:outline-none focus:ring-0" type="text" placeholder="Create new list" value={state.value} disabled={isBusy || doRetry} />
      ...
      {state.mode === MODE_EDIT && <button onTouchStart={onOkBtnPress} onMouseDown={onOkBtnPress} onClick={onOkBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-center items-center w-10 h-10 group focus:outline-none">
        <svg className="w-4 text-gray-500 rounded-sm group-hover:text-gray-600 group-focus:ring-2 group-focus:ring-offset-4 group-focus:ring-gray-400" viewBox="0 0 14 10" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      {(state.mode === MODE_VIEW && listNameObj !== null && !doRetry) && <button onClick={onEditBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-center items-center w-10 h-10 group focus:outline-none" disabled={isBusy}>
        <svg className="w-4 h-4 text-gray-500 rounded-sm group-hover:text-gray-600 group-focus:ring-2 group-focus:ring-offset-4 group-focus:ring-gray-400" viewBox="0 0 14 14" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      {(state.mode === MODE_VIEW && listNameObj !== null && doRetry) && <button onClick={onRetryRetryBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-center items-center w-20 h-10 focus:outline-none group">
        <div style={{ height: '1.75rem', paddingLeft: '0.625rem', paddingRight: '0.625rem' }} className="flex justify-center items-center bg-white border border-gray-300 rounded-md shadow-sm group-hover:border-gray-400 group-focus:border-gray-400 group-focus:ring-2 group-focus:ring-offset-2 group-focus:ring-gray-400">
          <span className="text-sm text-gray-500 group-hover:text-gray-600">Retry</span>
        </div>
      </button>}
      {(state.mode === MODE_VIEW && listNameObj !== null && doRetry) && <button onClick={onRetryCancelBtnClick} className="flex-grow-0 flex-shrink-0 flex justify-center items-center h-10 group focus:outline-none">
        <div className="px-1.5 py-0.5 text-sm text-gray-500 rounded group-hover:text-gray-600 group-focus:text-gray-600 group-focus:bg-gray-100">Cancel</div>
      </button>}
    </div>
  );
};
