...
const NoteEditorRetry = (props) => {
  ...
  return (
    <div className={tailwind('h-full w-full overflow-auto bg-white')}>
      <div style={style} className={tailwind('relative px-4 pb-4 sm:px-6 sm:pb-6')}>
        <div className={tailwind('h-16 w-full')} />
        <h3 className={tailwind('pt-5 text-lg font-medium text-gray-800')}>Problem found</h3>
        <p className={tailwind('text-sm font-normal text-gray-500')}>Please wait a moment and try again. If the problem persists, please <a className={tailwind('rounded-sm underline hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-400')} href={'/' + HASH_SUPPORT} target="_blank" rel="noreferrer">contact us
        ...
        <div className={tailwind('flex items-center pt-6')}>
          ...
          <button onClick={onRetryCancelBtnClick} className={tailwind('ml-2 rounded-md border border-white bg-white px-2 py-1.5 text-sm text-gray-500 hover:text-gray-700 focus:bg-gray-200 focus:text-gray-700 focus:outline-none')}>Cancel</button>
        </div>
        <div className={tailwind('mt-6 rounded-lg border border-gray-200 px-4 py-5')}>
