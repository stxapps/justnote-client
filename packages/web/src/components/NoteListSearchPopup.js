...
const NoteListSearchPopup = () => {
  ...
  return (
    <AnimatePresence key="AP_NL_SearchPopup">
      <motion.div className={tailwind('flex h-14 items-center justify-between border-b border-gray-200 pl-4 pr-2 sm:pl-6 sm:pr-4 lg:hidden')} variants={popupFMV} initial="hidden" animate="visible" exit="hidden">
        <label htmlFor="search" className={tailwind('sr-only')}>Search</label>
        <div className={tailwind('relative flex-1 rounded-md bg-white shadow-sm')}>
          <div className={tailwind('pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3')} aria-hidden="true">
            <svg className={tailwind('mr-3 h-4 w-4 text-gray-400')} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
              <path fillRule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clipRule="evenodd" />
            </svg>
          </div>
          <input ref={searchInput} onChange={onSearchInputChange} type="search" name="search" id="search" className={tailwind('block w-full rounded-md border border-gray-300 py-1.5 pl-9 pr-6 text-base leading-5 text-gray-700 focus:border-gray-500 focus:ring-gray-500')} placeholder="Search" value={searchString} autoCapitalize="none" />
          <button onClick={onSearchClearBtnClick} className={tailwind(`group absolute inset-y-0 right-0 flex items-center pr-2 focus:outline-none ${searchClearBtnClasses}`)}>
            <svg className={tailwind('h-5 cursor-pointer rounded-full text-gray-400 group-hover:text-gray-500 group-focus:text-gray-500 group-focus:ring-2 group-focus:ring-gray-400')} viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <button onClick={onSearchCancelBtnClick} className={tailwind('ml-2 flex-shrink-0 flex-grow-0 rounded-md px-2 py-1.5 text-sm text-gray-500 hover:text-gray-700 focus:bg-gray-200 focus:text-gray-700 focus:outline-none')}>Cancel</button>
      </motion.div>
    </AnimatePresence>
  );
};
