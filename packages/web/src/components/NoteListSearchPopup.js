...
const NoteListSearchPopup = () => {
  ...
  return (
    <AnimatePresence key="AP_NL_SearchPopup">
      <motion.div className="h-14 border-b border-gray-200 flex items-center justify-between pl-4 pr-2 sm:pl-6 sm:pr-4 lg:hidden" variants={popupFMV} initial="hidden" animate="visible" exit="hidden">
        <label htmlFor="search" className="sr-only">Search</label>
        <div className="flex-1 relative rounded-md bg-white shadow-sm">
          ...
          <input ref={searchInput} onChange={onSearchInputChange} type="search" name="search" id="search" className="focus:ring-gray-500 focus:border-gray-500 block w-full pl-9 pr-6 py-1.5 text-base leading-5 text-gray-700 border border-gray-300 rounded-md" placeholder="Search" value={searchString} autoCapitalize="none" />
        ...
        <button onClick={onSearchCancelBtnClick} className="ml-2 px-2 py-1.5 flex-grow-0 flex-shrink-0 text-sm text-gray-500 rounded-md hover:text-gray-700 focus:outline-none focus:text-gray-700 focus:bg-gray-200">Cancel</button>
      </motion.div>
    </AnimatePresence>
  );
};
