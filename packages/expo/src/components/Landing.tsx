    ...
    <React.Fragment>
      <ScrollView contentContainerStyle={tailwind('min-h-full')}>
        ...
        <View style={tailwind('items-center justify-center p-6 lg:p-8')}>
          <TouchableOpacity onPress={onSignUpBtnClick} style={tailwind(`flex-row items-center justify-center rounded-md border border-transparent bg-green-600 px-8 py-3 shadow md:py-4 md:px-10 ${safeAreaWidth < toPx(SM_WIDTH) ? 'w-full' : ''}`)}>
          ...
          <TouchableOpacity onPress={onAsDummyBtnClick} style={tailwind(`items-center justify-center border border-transparent bg-transparent px-8 py-3 md:py-4 md:px-10 ${safeAreaWidth < toPx(SM_WIDTH) ? 'w-full' : ''}`)}>
