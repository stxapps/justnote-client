  ...
  return (
    <View style={tailwind('p-4 md:p-6')}>
      <View style={tailwind('border-b border-gray-200 blk:border-gray-700 md:hidden')}>
        ...
        <Text style={tailwind('pb-2 text-xl font-medium leading-6 text-gray-800 blk:text-gray-100')}>Lists</Text>
      </View>
      <View style={tailwind('hidden md:flex')}>
        <Text style={tailwind('text-base font-medium leading-5 text-gray-800 blk:text-gray-100')}>Lists</Text>
      </View>
    ...
    <React.Fragment>
      <View style={[tailwind('mt-1 flex-row items-center justify-start'), viewStyle]}>
        ...
        <View style={tailwind('flex-shrink flex-grow')}>
          <TextInput ref={input} onFocus={onInputFocus} onBlur={onInputBlur} onChange={onInputChange} onSubmitEditing={onInputKeyPress} style={tailwind(`w-full border-0 bg-transparent px-0 text-base font-normal text-gray-600 blk:text-gray-300 ${textInputClasses}`)} placeholder="Create a new list" placeholderTextColor={themeMode === BLK_MODE ? 'rgb(156, 163, 175)' : 'rgb(107, 114, 128)'} value={state.value} editable={!isBusy} />
